...
public class CaptureHelper extends ScreenRecorder{
    ...
    public static void startRecord(String recordName) {
        ...
        File file = new File(SystemHelper.getCurrentDirectory() + PropertiesHelper.getValue("VIDEO_RECORD_PATH"));
    ...
    public static void captureScreenshot(String screenshotName) {
        try {
            ...
            File theDir = new File(SystemHelper.getCurrentDirectory() + PropertiesHelper.getValue("SCREENSHOT_PATH"));
            if (!theDir.exists()) {
                theDir.mkdirs();
            }
            // Chổ này đặt tên thì truyền biến "screenName" gán cho tên File chụp màn hình
            FileHandler.copy(source, new File(SystemHelper.getCurrentDirectory() + PropertiesHelper.getValue("SCREENSHOT_PATH") + File.separator + screenshotName + "_" + dateFormat.format(new Date()) + ".png"));
    ...
    public static void takeScreenshoot(String screenshootName) {
        ...
        File theDir = new File(SystemHelper.getCurrentDirectory() + PropertiesHelper.getValue("SCREENSHOT_PATH"));
        if (!theDir.exists()) {
            theDir.mkdirs(); //tao moi thu muc
        }
        //Lưu file ảnh với tên cụ thể vào đường dẫn
        try {
            FileHandler.copy(source, new File(SystemHelper.getCurrentDirectory() + PropertiesHelper.getValue("SCREENSHOT_PATH") + screenshootName + ".png"));
            System.out.println("Take screenshot " + screenshootName + " successfully");
        } catch (IOException e) {
            System.out.println("ERROR. Can not take screenshot " + screenshootName);
        }
    }
}
