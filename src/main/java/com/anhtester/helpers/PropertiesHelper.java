...
public class PropertiesHelper {
    ...
    //ham loadAllfile() de doc nhieu file properties 1 luot
    public static Properties loadAllFiles() {
        ...
        try {
            ...
            for (String f : files) {
                ...
                tempProp.load(file);
                properties.putAll(tempProp); //doc tat ca du lieu cua cac file gehp lai voi nhau
            }
            return properties;
    ...
    public static void setValue(String key, String keyValue) {
        try {
            if (file == null) {
                properties = new Properties();
                file = new FileInputStream(SystemHelper.getCurrentDirectory() + relPropertiesFilePathDefault); //doc file
                properties.load(file);
                file.close();
                out = new FileOutputStream(SystemHelper.getCurrentDirectory() + relPropertiesFilePathDefault); //ghi file
            }
            //Ghi vào cùng file Prop với file lấy ra
            out = new FileOutputStream(linkFile);
            System.out.println(linkFile);
            properties.setProperty(key, keyValue);
            properties.store(out, null); //luu gia tri vua set vao file
            out.close();
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
